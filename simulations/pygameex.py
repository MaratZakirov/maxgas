...
from sympy.polys.heuristicgcd import HEU_GCD_MAX
...
class Ball:
    def __init__(self, x, y, radius, speed_x, speed_y):
        ...
        self.vx = speed_x
        self.vy = speed_y
        self.E_init = (HEIGHT - self.y)*GRAVITY + 0.5*(self.vx**2 + self.vy**2)
    ...
    def energy_correction(self):
        ...
        #self.vx *= coef
        #self.vy *= coef
        E_cur = (HEIGHT - self.y)*GRAVITY + 0.5*(self.vx**2 + self.vy**2)
...
for _ in range(1000):
    ...
    balls.append(Ball(x, y, radius, speed_x, speed_y))
...
#print(np.quantile(h, q=0.2), np.quantile(h, q=0.8))
#plt.hist(h, bins=10)
#plt.show()

H_min = np.quantile(h, q=0.2)
H_max = np.quantile(h, q=0.8)
